...
function video_extends_admin_menu() {
    ...
    add_menu_page(
        __( 'Video Extends', 'video-extends' ),         
        __( 'Video Extends', 'video-extends' ),          
        'manage_options',                               
        'video-extends',                                
        'welcome',              
    ...
    add_submenu_page(
        'video-extends',                                 
        __( 'Video Extends Documentation', 'video-extends' ),  
        __( 'Documentation', 'video-extends' ),         
        'manage_options',                                
        'video-extends-documentation',                   
...
function welcome() {
    echo "**************************************************************";
    echo "<h1>" . esc_html__( 'Enjoy Video Extends!', 'video-extends' ) . "</h1>";
    echo "**************************************************************";
    echo "<br>";
    echo "<h1>";
    echo "😊😄😄😄😄😄😄😄😄😄😄😄😄😄😄😄😄😄😄😄😄😄😄";
...
require_once plugin_dir_path(__FILE__) . 'functions/functions.php';
    ...
    return preg_replace_callback('/\[video_extends.*?\]/s', function($matches) {
        return ve_clean_shortcode_full($matches[0]);
    }, $content);
});
    add_filter('the_content', function($content) {
    $content = preg_replace('/<p>\s*(\[video_extends[^\]]*\])\s*<\/p>/i', '$1', $content);
    return $content;
});
    // filtros para llamar la funcion de eliminar espacio
    add_filter('the_content', 've_force_video_inline', 5);

// Shortcode con parámetros [video_extends video="Sintel.mp4" width="800" height="450" autoplay="true" loop="true"]
function video_extends_shortcode($atts) {
   ...
   // Parámetros del shortcode con valores por defecto
    ...
    if (!file_exists(plugin_dir_path(__FILE__) . 'assets/video/' . $atts['video'])) {
       return '<p>' . __('Video not found.', 'video-extends') . '</p>';
    ...
    $markers_array = array(
        array('time' => ve_convert_to_seconds('0:39'), 'text' => __('Chapter 1', 'video-extends')),
        array('time' => ve_convert_to_seconds('5:50'), 'text' => __('Chapter 2', 'video-extends')),
        array('time' => ve_convert_to_seconds('7:50'), 'text' => __('Chapter 3', 'video-extends')),
        array('time' => ve_convert_to_seconds('11:17'), 'text' => __('Chapter 4', 'video-extends')),
    );
}
    ...
    ob_start(); ?>
     <div class="container">
        <video id="<?php echo esc_attr($atts['id']);  ?>" width="<?php echo esc_attr($atts['width']);?>" height="<?php echo esc_attr($atts['height']); ?>" poster="<?php echo esc_url( $poster_url); ?>" controls  <?php echo esc_attr( $video_attrs); ?> >
...
add_shortcode('video_extends', 'video_extends_shortcode');
